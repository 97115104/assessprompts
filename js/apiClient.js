const ApiClient = (() => {
    ...
    async function assess({ apiKey, baseUrl, model, systemMessage, userMessage, apiMode, puterModel, ollamaUrl, ollamaModel }) {
        if (apiMode === 'puter') {
    ...
    async function callPuterApi({ model, systemMessage, userMessage }) {
        ...
        try {
            ...
            if (msg.includes('context_length') || msg.includes('too long') || msg.includes('max_tokens')) {
                throw new Error(
                    'Prompt is too long for this model. Try shortening your prompt or switching to gpt-oss-120b.'
                );
            }
    ...
    async function callOpenRouterApi({ base, apiKey, modelName, systemMessage, userMessage }) {
        ...
        const headers = {
            ...
            'HTTP-Referer': window.location.origin || 'https://assessprompts.app',
            'X-Title': 'Assess Prompts'
        };
    ...
    function parseResponse(content) {
        ...
        // Fallback: return minimal assessment with raw content
        return {
            assessment_summary: content.substring(0, 500),
            score: 0,
            grade: '?',
            strengths: [],
            issues: ['Could not parse structured response from the model. The raw response is shown below.'],
            missing_elements: [],
            suggestions: [],
            optimized_version: '',
            token_count: Math.ceil(content.split(/\s+/).length * 1.3),
            estimated_output_tokens: 0,
            cost_estimates: { models: [], self_hosted_note: '' },
            optimization_notes: ''
        };
    }
